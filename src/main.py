...
def load_data_optimizer(config):
    ...
    data_loader = config.init_obj(
        "data_loader",
        data_loaders_,
        **{"training": True, "label_name": config["label_name"]}
    )
    ...
    search_method_params = {"estimator": model, "scoring": scoring, "cv": cross_val}
...
def main(config):
    ...
    if config["test_model"]:
        data_loader = config.init_obj(
            "data_loader",
            data_loaders_,
            **{"training": False, "label_name": config["label_name"]}
        )
