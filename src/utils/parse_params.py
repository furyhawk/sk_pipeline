...
def modify_params(search_method_params, config):
    ...
    assert (
        ...
        or search_type == "RandomizedSearchCV"
        or search_type == "HalvingGridSearchCV"
        or search_type == "HalvingGridSearchCV"
        or search_type == "BayesSearchCV"
    ...
    if "Grid" in search_type:
        ...
        for method_name in tuned_parameters[0]:
            ...
            if len(temp) == 3 and temp[0] == "RS":
                tuned_parameters[0][method_name] = Real(
                    temp[1], temp[2], prior="log-uniform"
                )
            elif len(temp) == 3 and temp[0] == "RSI":
                tuned_parameters[0][method_name] = Integer(
                    temp[1], temp[2], prior="uniform"
                )
            else:
...
def get_lib(search_type):
    if (
        ...
        or search_type == "RandomizedSearchCV"
        or search_type == "HalvingGridSearchCV"
        or search_type == "HalvingRandomSearchCV"
    ):
